-- AI Tools Enhancements for Plugin Marketplace
-- Adds advanced AI tools management with configuration, tier-based access, and lifecycle management

-- Add enhanced columns to existing marketplace_ai_tools table
DO $$
BEGIN
    ...
    IF NOT EXISTS (
        ...
        ADD COLUMN required_tier VARCHAR(50) DEFAULT 'community',
...
DROP CONSTRAINT IF EXISTS marketplace_ai_tools_registrations_activation_status_check;
...
CREATE INDEX IF NOT EXISTS idx_ai_tools_enabled
ON marketplace_ai_tools(is_enabled)
WHERE is_enabled = TRUE;
