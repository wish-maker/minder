...
class NativeGraphRAG:
    ...
    async def connect(self):
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                _ = await client.get(f"{self.neo4j_http}/db/data/neo4j/tx/commit")
    ...
    async def retrieve_graph_context(
        self, query: str, traversal_depth: int = 2, limit: int = 15
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            keyword_match = " OR ".join(
                [f"e.name = '{kw}'" for kw in query_keywords[:3]]
            )
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if response.status_code == 200:
                        ...
                        for record in result["results"][0].get("data", []):
                            if len(record) >= 4:
                                entity_contexts.append(
                                    {
                                        "entity": record[0],
                                        "type": (
                                            record[1] if len(record) > 1 else "keyword"
                                        ),
                                        "relationships": (
                                ...
                                related_entities.append(
                                    record[0] if len(record) > 3 else ""
                                )

                    logger.info(
                        f"✅ Native Graph RAG: Retrieved {len(entity_contexts)} entity contexts"
                    )
                    return {
                    ...
                    logger.warning(
                        f"⚠️ Native Graph RAG: Query failed with status {response.status_code}"
                    )
                    return {"entity_contexts": [], "related_entities": []}
