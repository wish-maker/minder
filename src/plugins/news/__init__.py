...
Config (env on plugin-registry; all optional — keyless defaults):
  NEWS_FEEDS          ``name:url`` pairs, comma-sep (url may contain ':', split once).
                      Default HackerNews + BBC front page.
...
_DEFAULT_FEEDS = (
    "hackernews:https://hnrss.org/frontpage," "bbc:http://feeds.bbci.co.uk/news/rss.xml"
)
...
class NewsPlugin:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.feeds = self._parse_feeds(os.environ.get("NEWS_FEEDS", _DEFAULT_FEEDS))
        self.max_items = int(os.environ.get("NEWS_MAX_ITEMS", "10"))
        self.sink_influxdb = os.environ.get("NEWS_SINK_INFLUXDB", "1") == "1"
        self.http_timeout = float(os.environ.get("NEWS_HTTP_TIMEOUT", "10"))
