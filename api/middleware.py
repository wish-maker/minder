...
logger.info(f"✅ Rate limiters initialized (Standard: 50/hour public, Expensive: 5/minute)")
logger.info(f"   - Local network: Unlimited (standard), 30/minute (expensive)")
logger.info(f"   - VPN: 200/hour (standard), 10/minute (expensive)")
logger.info(f"   - Public: 50/hour (standard), 5/minute (expensive)")
...
def setup_middleware(app, allowed_origins: list):
    ...
    # Instead, rate limiting will be applied selectively via endpoint decorators
