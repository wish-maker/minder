...
class NewsModule(BaseModule):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.feeds = config.get('news', {}).get('feeds', [
            {
                'name': 'BBC World',
    ...
    async def register(self) -> ModuleMetadata:
        ...
        logger.info(f"📰 Registering News Module")
    ...
    def _parse_rss_xml(self, xml_content: str, source: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if len(items) > 0:
                ...
                logger.info(f"First item title element: tag={title_elem.tag}, text={repr(title_elem.text)}, has_children={len(list(title_elem)) > 0}")
            ...
            for idx, item in enumerate(items):
                try:
                    ...
                    if not title or len(title.strip()) < 10:
                        logger.warning(f"Item {idx}: Skipping article with no/short title from {source}. Title: '{title[:50] if title else '(empty)'}', length={len(title)}")
                        continue
                    ...
                    if pub_date_str:
                        for fmt in ['%a, %d %b %Y %H:%M:%S', '%Y-%m-%dT%H:%M:%S%z']:
                            try:
