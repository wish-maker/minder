...
# Frozen bash reference for the behavior gate (#7 complete — the live entrypoint is
# now the Python shim `setup.sh`). SCRIPT_NAME is pinned to "setup.sh" (the user-
# facing command) so hint messages + the gate baseline stay identical to the port.
