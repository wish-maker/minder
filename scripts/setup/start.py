...
def run() -> int:
    ...
    # AI_COMPUTE_MODE / COMPUTE_RESOURCE_PROFILE, or external AI mode with no URL)
    # aborts before the stack comes up. Mirror that — don't boot a half-configured
    # stack after a config error was logged.
    if (
        preflight.validate_ai_compute_mode() != 0
