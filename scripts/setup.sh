...
readonly -a AI_SERVICES=(openwebui tts-service model-fine-tuning)
...
compose() {
    # Auto-enable secrets if available (production security best practice)
    if [[ -f "$SECRETS_FILE" ]] && [[ -d "${SCRIPT_DIR}/.secrets" ]] && [[ -n "$(ls -A ${SCRIPT_DIR}/.secrets/*.secret 2>/dev/null)" ]]; then
...
_fill_env_secrets() {
    ...
    while IFS= read -r line; do
        if [[ "$line" =~ ^([A-Z_]+)=(CHANGEME|REPLACE_ME|CHANGE_ME.*|)$ ]]; then
...
_validate_env() {
    ...
    if [[ ! -f "$ENV_EXAMPLE" ]]; then
        log_warn ".env.example not found - may affect default configuration"
    fi
}
...
# ─────────────────────────────────────────────────────────────
# INFLUXDB SECRETS FOR TELEGRAF
# ─────────────────────────────────────────────────────────────

_create_influxdb_secrets() {
    local secret_file="${SCRIPT_DIR}/.secrets/influxdb.env"
    local secret_dir
    secret_dir="$(dirname "$secret_file")"

    # Check if secret file already exists
    if [[ -f "$secret_file" ]]; then
        log_detail "InfluxDB secrets already exist: ${secret_file}"
        return 0
    fi
    ...
    if [[ ! -d "$secret_dir" ]]; then
        mkdir -p "$secret_dir"
        log_detail "Created secrets directory: ${secret_dir}"
    fi

    # Get values from .env
    local influxdb_token influxdb_org influxdb_bucket
    local influxdb_host influxdb_username influxdb_password

    influxdb_token="$(_env_get "INFLUXDB_TOKEN")"
    influxdb_org="$(_env_get "INFLUXDB_ORG")"
    influxdb_bucket="$(_env_get "INFLUXDB_BUCKET")"
    influxdb_username="$(_env_get "INFLUXDB_ADMIN_USERNAME")"
    influxdb_password="$(_env_get "INFLUXDB_ADMIN_PASSWORD")"

    # Set defaults if not found
    influxdb_org="${influxdb_org:-minder}"
    ...
    # Create secret file
    cat > "$secret_file" << EOF
# InfluxDB v3 Configuration for Telegraf
# Generated: $(date -u '+%Y-%m-%dT%H:%M:%SZ')
# ⚠  Keep this file secret. Never commit to version control.

# InfluxDB v3 connection settings
...
EOF
    ...
    log_success "Created InfluxDB secrets: ${secret_file}"
    log_detail "Telegraf can now connect to InfluxDB"
}
...
check_port_conflicts() {
    ...
    local -a critical_ports=(80 443 3000 5432 6379 9090 9093 8086 11434)
    ...
    if (( ${#conflicting_ports[@]} > 0 )); then
        log_warn "Ports already in use: ${conflicting_ports[*]}"
        log_detail "This may cause service startup failures"
        log_detail "Stop conflicting services before proceeding"
        return 1
    else
...
create_external_volumes() {
    log_step "Creating external Docker volumes"

    readonly -a EXTERNAL_VOLUMES=(
        postgres_data
        redis_data
        minio_data
        ollama_data
    ...
    if (( created > 0 )); then
        log_success "${created} external volume(s) created"
    fi
    if (( existing > 0 )); then
        log_info "${existing} external volume(s) already exist"
    fi
}
...
readonly -a API_DATABASES=(api_gateway_db plugin_registry_db rag_pipeline_db model_management_db)
...
initialize_database() {
    ...
    log_info "Creating API databases…"
    for db in "${API_DATABASES[@]}"; do
        if docker exec "$(container_name postgres)" psql -U minder \
               -c "CREATE DATABASE ${db};" &>/dev/null 2>&1; then
            log_detail "Created: ${db}"
        else
...
initialize_minio() {
    ...
    if ! docker exec minder-minio which mc &>/dev/null; then
        ...
        # Download mc locally first, then copy to container
        local mc_tmp="/tmp/mc-minio"
        wget -q https://dl.min.io/client/mc/release/linux-amd64/mc -O "$mc_tmp" 2>/dev/null || \
          curl -sL https://dl.min.io/client/mc/release/linux-amd64/mc -o "$mc_tmp"
        chmod +x "$mc_tmp"
        docker cp "$mc_tmp" minder-minio:/tmp/mc
...
start_services_manually() {
    ...
    # TTS/STT Service
    if ! container_running tts-service; then
        log_detail "Starting TTS/STT Service..."
        run docker run -d \
          --name "$(container_name tts-service)" \
          --network "$NETWORK_NAME" \
          -p 8006:8006 \
...
run_health_checks() {
    ...
    [[ "$json_mode" == false ]] && echo -e "\n${BOLD}AI Services${NC}"
    for svc in openwebui tts-service model-fine-tuning; do
        [[ -v "SERVICE_PORTS[$svc]" ]] && _check_endpoint "$svc" "${SERVICE_PORTS[$svc]}"
    done
...
cmd_migrate() {
    ...
    for svc in "${migration_services[@]}"; do
        if container_running "$svc"; then
            ...
            if docker exec "$(container_name "$svc")" which alembic &>/dev/null; then
                ...
                log_detail "${svc} — alembic not found, skipping (database already created)"
                # Database already created in initialize_database(), so we're good
                log_success "${svc} — database ready (migration skipped)"
            fi
        else
...
cmd_regenerate_compose() {
    ...
    local compose_template="${SCRIPT_DIR}/templates/docker-compose.yml.template"
    local compose_output="${SCRIPT_DIR}/infrastructure/docker/docker-compose.yml"
...
EOF
	...
	post_install_system_config() {
	    section "🔧 System Configuration"

	    log_info "Applying critical system fixes for zero-install deployment..."

	    # 1. Fix UFW FORWARD policy for Docker container access
	    if command -v ufw &>/dev/null && ufw status | grep -q "Status: active"; then
	        ...
	        if [[ "$current_policy" == "DROP" ]]; then
	            log_detail "Fixing UFW FORWARD policy (DROP → ACCEPT)..."
	            run sed -i 's/DEFAULT_FORWARD_POLICY="DROP"/DEFAULT_FORWARD_POLICY="ACCEPT"/' /etc/default/ufw
	            run ufw reload &>/dev/null
	            log_success "UFW FORWARD policy updated"
	        else
	            log_detail "UFW FORWARD policy is ACCEPT (OK)"
	        fi
	    else
	        log_detail "UFW not active, skipping..."
	    fi

	    # 2. Update Telegraf config for InfluxDB v2 compatibility
	    local telegraf_conf="${SCRIPT_DIR}/infrastructure/docker/telegraf/telegraf.conf"
	    if [[ -f "$telegraf_conf" ]] && grep -q "\[\[outputs.influxdb_v3\]\]" "$telegraf_conf"; then
	        log_detail "Updating Telegraf config (InfluxDB v3 → v2)..."
	        run cp "$telegraf_conf" "${telegraf_conf}.backup"
	        run sed -i 's/\[\[outputs.influxdb_v3\]\]/[[outputs.influxdb_v2]]/' "$telegraf_conf"
	        run sed -i 's/^  database = "minder_metrics"/  organization = "${INFLUXDB_ORG:-minder}"/' "$telegraf_conf"
	        run sed -i '/^  database = "minder_metrics"/d' "$telegraf_conf"
	        run sed -i '/^  organization/a\  bucket = "${INFLUXDB_BUCKET:-telegraf}"' "$telegraf_conf"
	        log_success "Telegraf config updated"
	    else
	        log_detail "Telegraf config already correct"
	    fi

	    # 3. Fix Redis exporter authentication format
	    local compose_file="${SCRIPT_DIR}/infrastructure/docker/docker-compose.yml"
	    if [[ -f "$compose_file" ]] && grep -q "REDIS_ADDR=redis://:\${REDIS_PASSWORD}@redis:6379" "$compose_file"; then
	        log_detail "Fixing Redis exporter auth format..."
	        run sed -i 's/REDIS_ADDR=redis://:\${REDIS_PASSWORD}@redis:6379/REDIS_ADDR=redis:\/\/redis:6379/' "$compose_file"
	        run sed -i '/REDIS_ADDR=redis:\/\/redis:6379/a\    - REDIS_PASSWORD=${REDIS_PASSWORD}' "$compose_file"
	        log_success "Redis exporter config updated"
	    else
	        log_detail "Redis exporter config already correct"
	    fi
...
cmd_install() {
    ...
    progress_init 12   # one extra step for external volumes

    progress_next "Checking prerequisites";      check_prerequisites
    progress_next "Setting up environment";     setup_environment
    progress_next "Checking port conflicts";     check_port_conflicts
    progress_next "Creating external volumes";   create_external_volumes
    ...
    progress_next "System configuration";         post_install_system_config
