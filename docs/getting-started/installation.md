...
- 🔁 **Env mirror:** Copies root `./.env` → `docker/.env` (the path Compose reads)
...
Root `./.env` is the **single source of truth**. `docker/.env` is auto-generated by `setup.sh` — do not edit it directly.
...
bash setup.sh bundle enable|disable <name>        # Toggle a capability bundle (add --stop-orphans to also stop services no longer claimed)
...
If you have port conflicts, edit the ports in `docker/docker-compose.yml`:
...
# Force rebuild
docker compose --file docker/docker-compose.yml build --no-cache <service>
```
...
> Authelia SSO is **enabled and enforcing** forward-auth on 6 Traefik routers (minio,
