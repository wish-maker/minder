...
| POST | `/pipeline` | Create a RAG pipeline over one or more knowledge bases. The creator (JWT `sub`) is recorded as `owner_user_id` for owner-scoping (#943) |
| GET | `/pipeline` | List RAG pipelines (#426) — paginated via `limit`/`offset`; returns the shared `{items, total, limit, offset}` envelope (#501). Optional `?owner_user_id=` filters to that owner's pipelines (+ legacy null-owner ones), used by the gateway to owner-scope the `ask_<pipeline>` chat tools (#943) |
