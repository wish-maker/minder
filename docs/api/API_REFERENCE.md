...
The Minder Platform provides RESTful APIs through a central API Gateway. All external requests go through the gateway, which handles authentication, rate limiting, routing, and logging.

**API Architecture:**
```
Client → API Gateway (Port 8000) → Plugin Registry (Port 8001)
